...
- **Unix sockets are outside the policy entirely, on both platforms.**
  Landlock mediates TCP, not AF_UNIX, so a sandboxed process reaches any
  unix socket the filesystem permissions allow — including ones in
  ungranted directories. Seatbelt is no better: measured on macOS 15, the
  same ungranted socket connects there too. On a host with a nix daemon
  that means the sandbox holds whatever authority that daemon extends to a
  local user; on a host with a Docker socket it would mean far more.
  Measured in `tests/unix_socket_not_mediated.rs`; see
  `docs/known-gaps.md`. This is a property of the mechanism, not an
  oversight in the policy, and closing it needs a mount namespace rather
  than a Landlock rule.
