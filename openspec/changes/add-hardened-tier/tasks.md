...
- [ ] 1.1 Add an `isolation` field to `Sandbox` in `src/config/mod.rs`
      (`"process"` default | `"hardened"`), parsed the same way
      neighboring `[sandbox]` fields are
- [ ] 1.2 Reject unrecognized `isolation` values through the existing
      `ConfigError` enum, consistent with other manifest validation
      errors
- [ ] 1.3 Unit tests: omitted key defaults to `process`; explicit
      `hardened` parses; an invalid value produces `ConfigError`

## 2. Keeper: session backend abstraction

- [ ] 2.1 Extract the body of `keeper::session::spawn()` into a
...
- [ ] 2.2 `Keeper::new` takes `Arc<dyn SessionBackend>` instead of
      assuming local spawn; `connection.rs`/`pty.rs`/`protocol.rs`/
      `registry.rs` stay unchanged — they never touch the spawn
      mechanism directly
- [ ] 2.3 Route the ssh server's own session spawning through the same
...
- [ ] 2.4 Existing keeper/session integration tests still pass unmodified
...
- [ ] 3.1 Resolve the isolation tier and concrete backend at `up` from
...
- [ ] 3.2 `process` tier path: today's code, unchanged — nono profile,
      fd-inheritance, `SessionBackend = Local`
- [ ] 3.3 `hardened` tier path: delegate bundle synthesis and sandbox
...
- [ ] 3.4 `src/lifecycle/status.rs`: add a tier/backend field to
      `SandboxStatus` so `status` prints `isolation: process` or
      `isolation: hardened (<backend>/<platform>)`
- [ ] 3.5 `doctor` in `src/bin/devcroft.rs`: add a generic
...
- [ ] 4.1 Confirm `policy::compile`, `render`, and `why` remain fully
...
- [ ] 4.2 Add the per-tier projection dispatch point in `up` (nono
...
- [ ] 5.1 Amend CLAUDE.md's "SSH lives inside the boundary" invariant to
...
- [ ] 5.3 `openspec validate --all` passes with this change's `tasks.md`
