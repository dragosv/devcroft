...
devcroft supervises devenv's `processes` — but that is a different
subject, and it has its own sample:
[samples/devenv-services-sample](../devenv-services-sample/). It covers
the port grant, the ordering spelling that looks right and is not, what
is refused rather than dropped, and where a service you never wrote comes
from.

Kept out of this one deliberately. The subject here is the hook, and a
sample that demonstrates two things demonstrates neither as clearly.
