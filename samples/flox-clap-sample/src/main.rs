// citytime: a two-subcommand CLI demonstrating clap's derive API — the
// de facto standard for a Rust CLI with more than a couple of subcommands
// (structopt merged into clap v3; there is no reason to reach for it
// separately anymore). #[derive(Parser)]/#[derive(Subcommand)] give you a
// working --help, per-subcommand help, and (via clap_complete, not wired up
// here) shell completions for free from the same struct that parses args.
...
/// consistent with samples/flox-rustup-sample's whole point about what
/// belongs at host-side provisioning versus inside a sandboxed session.
